...
async function loadHomepageDatabase() {
    ...
    try {
        ...
        recommendedList.forEach(anime => {
            recommendedContainer.insertAdjacentHTML("beforeend", generateCardHtml(anime));
        });
...
async function executeAnimeSearch(keyword) {
    ...
    try {
        const response = await fetch("https://anilist.co", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Accept": "application/json"
...
// Trigger 1: Pag-click sa magnifying glass icon 🔍
